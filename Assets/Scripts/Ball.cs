    ...
    private void FixedUpdate()
    {
        ...
        if (hit.collider != null)
        {
            ...
            if(goalPost != null)
            {
                ...
                {
                    GameManager.Instance.AddScore( playerNumber: 2, score: 1);
                }
                else if (goalPost.playerNumber == 2)
                {
                    GameManager.Instance.AddScore(playerNumber: 1, score: 1);
                }
            }

            direction = Vector2.Reflect(direction, hit.normal);
            direction += (Vector2)(Random.insideUnitSphere * randomRefectionIntensity);
        }
